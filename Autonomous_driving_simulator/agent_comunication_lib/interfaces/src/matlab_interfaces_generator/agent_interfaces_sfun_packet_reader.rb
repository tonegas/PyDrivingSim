...
File.open("#{$out_dir_matlab}/#{$base_name}_packet_reader.c", "w") do |f|
  ...
  body = <<-EOS
	...
	#define S_FUNCTION_NAME #{$base_name}_packet_reader
	...
	#define OUTPUT_0_WIDTH        #{result[:input_size]}
	...
	*status = -1;
	if (*recv_bytes > 0) {
		...
		memcpy((void *) manoeuvre_array_out+((size_t) part_pos)*sizeof(packet.data_struct.datagram_part), &packet.data_struct.datagram_part, (size_t) part_size);
	#elif defined(_WIN32)
		memcpy((char *) manoeuvre_array_out+((size_t) part_pos)*sizeof(packet.data_struct.datagram_part), &packet.data_struct.datagram_part, (size_t) part_size);
	#endif
	...
	static void mdlOutputs(SimStruct *S, int_T tid)
	{
		...
		const uint32_T *readed_packets_in = (uint32_T *) ssGetInputPortRealSignal(S, 3);
		...
		agent_packet_reader_outputs(packet_array, recv_bytes, buffer_id, readed_packets_in, manoeuvre_array_out, status, stop_time, readed_packets_out, S);
	}
...
puts "File #{$out_dir_matlab}/#{$base_name}_packet_reader.c generated"
