...
#define S_FUNCTION_NAME interfaces_packet_creator

#define NUM_INPUTS 4
/* Input Port  0 */
#define IN_PORT_0_NAME scenario_array
#define INPUT_0_WIDTH 737
...
static void agent_packet_creator_output(const uint32_T *scenario_array,
                                        ...
                                        uint32_T *while_run, SimStruct *S) {
  packet_t packet;
  UDP_UINT n_packets;
  ...
  size_t scenario_array_size =
      (size_t)ssGetInputPortWidth(S, 0) * sizeof(uint32_T);
  ...
  swapBytes((void *)&packet.data_struct.server_run, (void *)server_run,
            sizeof(uint32_T), 1);
  swapBytes((void *)&packet.data_struct.part_pos, (void *)part_pos,
            sizeof(uint32_T), 1);
  swapBytes((void *)&packet.data_struct.last_part_pos, (void *)&last_part_pos,
            sizeof(uint32_T), 1);
  swapBytes((void *)&packet.data_struct.datagram_id, (void *)datagram_id,
            sizeof(uint32_T), 1);

  if (*part_pos < last_part_pos) {
    part_size = part_bytes;
    swapBytes((void *)&packet.data_struct.part_size, (void *)&part_bytes,
              sizeof(uint32_T), 1);
    *while_run = 0;
  } else {
    part_size = remaining_bytes;
    swapBytes((void *)&packet.data_struct.part_size, (void *)&remaining_bytes,
              sizeof(uint32_T), 1);
    *while_run = 1;
  }
...
/*  function call */
  ...
  usleep(1000 * TIME_SLEEP_MS); // 1000 factor used to convert from millisecond
                                // to microsecond
#endif
  agent_packet_creator_output(scenario_array, part_pos, server_run, datagram_id,
                              packet_array, while_run, S);
}
